...
function rStackedBarChart (settings) {
    ...
    this.render = function() {
        ...
        var y = d3.scale.linear()
          .rangeRound([height, 0]).clamp(true);
        ...
        var total = values[0].data.map(function() {return 0; });
        var seriesData = seriesNames.map(function (name, i) {
            return {
              name: name,
              values: values[i].data.map(function (d,j) {
                return {name: name, label: xAxisLabels[j], value:d, h0: total[j], h1: total[j]+d, total: total[j] += d };
              })
            };
        });
        ...
        series.selectAll(".rectangle")
          ...
          .enter().append("rect")
           ...
           .attr("y", function (d) { return y(d.h1)})
           .attr("width", x.rangeBand() + "px")
           .attr("height", function (d) { return y(d.h0) - y(d.h1); })
