...
function rBarChart (settings) {
    ...
    this.defaults = {
      ...
      yTitle: '',
      bar: {stroke: 'grey', strokeWidth: '0px'},
      path: {width: '2x'},
      innerPadding: '2'
    };
    var _this = this;
    ...
    this.render = function() {
        ...
        series.selectAll(".rectangle")
          .data(function (d) { return d.values; })
          .enter().append("rect")
           .attr("class", "rect")
           .attr("x", function (d,i) { return x(d.label) + seriesNames.indexOf(d.name)*x.rangeBand()/seriesNames.length })
           .attr("y", function (d) { return y(d.value)})
           .attr("width", x.rangeBand()/seriesNames.length - settings.innerPadding >= 1 ? x.rangeBand()/seriesNames.length - settings.innerPadding : 1 + "px")
           .attr("height", function (d) { return height - y(d.value); }) //return y(d.value - settings.margin.bottom);
           .style("fill", function (d) { return color(d.name); })
           .style("stroke", settings.bar.stroke)
           .style("stroke-width", settings.bar.strokeWidth)
